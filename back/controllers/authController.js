...
const registerController = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "user created successfully!", user: restUserData });
  } catch (err) {
    res
      .status(500)
      .json({ message: "an error accured in creating user", error: err });
  }
};
...
const logoutController = async (req, res) => {
  try {
    ...
    const refreshToken = await UserRefreshTokens.findOne({
      token: req.body.data,
    });
    !refreshToken &&
      res.status(200).send("token not found - logged out successfully!");
